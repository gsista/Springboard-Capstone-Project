---
title: "Springboard Capstone Project - Sample Datasets"
...
Run all the libraries before running the rest of the scripts
...
1a. Reading in the San Francisco Crime dataset sample

```{r}
crime_data <- read.csv("/Users/gsista/Desktop/Springboard/DataScience/Data/sf_crime_sample.tsv", sep = "\t")
crime_data
```
...
```{r}
library(dplyr)
colnames(sfmhp_latlong) <- c("X", "zip", "lat", "lon", "location_type", "formatted", "LatLong")
colnames(sfmhp_mhp_by_zip) <- c("zip", "median_house_price")
sfmhp_zip_latlong <- inner_join(sfmhp_mhp_by_zip, sfmhp_latlong, by = "zip")
sfmhp_zip_latlong
```
...
get_house_price <- function(LatLong) {
  return(sfmhp_zip_latlong[which(sfmhp_zip_latlong$LatLong == as.character(LatLong)), 'median_house_price'])
}
```
...
write.csv(combineddf, file = "/Users/gsista/Desktop/Springboard/DataScience/combineddata.csv")
```

Charts about crimes by weekday
